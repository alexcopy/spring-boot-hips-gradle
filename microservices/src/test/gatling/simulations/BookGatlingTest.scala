...
class BookGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Book entity")
        ...
        .repeat(2) {
            ...
            .get("/microservices/api/books")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new book")
            .post("/microservices/api/books")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "book":"SAMPLE_TEXT", "name":"SAMPLE_TEXT", "publishDate":"2020-01-01T00:00:00.000Z", "author":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_book_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created book")
                .get("/microservices${new_book_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created book")
            .delete("/microservices${new_book_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
